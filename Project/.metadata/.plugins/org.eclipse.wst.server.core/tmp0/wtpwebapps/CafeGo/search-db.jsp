...
%>
	<div style="margin: 80px 0 0 100px">
		<h2><span style="color: blue">"<%= input %>"</span>이(가) 포함된 카페명 검색 결과</h2>
<%
	int cnt;
	int count = 0;
	...
	try {
		...
		String sql = "select count(*) as cnt from cafe_tbl where name like '%" + input + "%'";
	...
	try {
		...
		conn = DriverManager.getConnection(jdbcurl, "project", "cafego");
		stmt = conn.createStatement();
		String sql = "select * from cafe_tbl where name like '%" + input + "%'";
		rs = stmt.executeQuery(sql);
	} catch (Exception e) {
		out.println("DB 연동 오류. : " + e.getMessage());
	}
